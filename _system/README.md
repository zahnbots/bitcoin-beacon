...
## Topic & archive pages (generated — 2026-07-20)
Top-menu order sitewide: the six beats → **Archive** → **Custody**. Each beat tab links straight to `/archive/<beat>.html` — that beat's **full** history, newest first (the newest stories sit at the top, so there's no separate "latest" view). The **Archive** menu item → `/archive.html`, a hub of six topic cards. `/topics/<beat>.html` are retired: they're now `noindex` redirect stubs → `/archive/<beat>.html` (kept only so any old links don't 404; not in the sitemap). All of `public/archive/*`, `public/archive.html`, and the `public/topics/*` stubs are GENERATED from `catalog.csv` by `_system/build-topic-pages.py` — never hand-edit them; edit the catalog (or the script's nav/shell) and re-run. Beats: on-the-ground, money-macro, markets, policy-nation-states, network-mining, opinion. NOTE: the hand-coded nav in index/story/custody/subscribe pages must stay identical to `nav_html()` in the script (six `/archive/<beat>.html` tabs + `/archive.html` Archive + `/custody/` Custody); if you change the nav, change it in the script AND sed it across those pages.

## Site upkeep files (owned by the daily run)
- `public/topics/<beat>.html` + `public/archive/<beat>.html` + `public/archive.html` — generated by `build-topic-pages.py` (see above); the daily run just re-runs the script after updating catalog.csv.
...
## Deploy (PUSH-ON-COMPLETION since 2026-07-27)
`public/` is the web root. Hosting: **Cloudflare Pages** project `bitcoin-beacon`, git-connected to **github.com/zahnbots/bitcoin-beacon** (private), production branch `main`, build command none, build output dir = `public`, serving **thebitcoinbeacon.com** (+ bitcoin-beacon.pages.dev preview). Pipeline: the daily run edits this folder and, as its FINAL site step, writes a timestamp to `_system/deploy.trigger` → a **launchd WatchPaths job** (`_system/auto-push.sh` via `~/Library/LaunchAgents/com.bitcoinbeacon.autopush.plist`, log at /tmp/bitcoinbeacon-push.log) fires on that file change, commits & pushes → Cloudflare auto-deploys in ~1 min. NO fixed-time push anymore — the old 30-min StartInterval used to snapshot the repo mid-run and publish half-built editions (new story pages, stale homepage). The daily run itself cannot push (sandbox has no github.com egress) — the launchd job owns that; for a manual push at any time, run `bash "/Users/mzahn/bitcoin beacon/_system/auto-push.sh"` or push from GitHub Desktop/Terminal. The old drag-drop Worker project `young-tooth-3a08` is retired (kept as fallback; safe to delete after a few clean days). www.thebitcoinbeacon.com still needs a DNS record + 301 redirect to the apex (one-time fix in the dashboard). Newsletter/subscribers: **beehiiv** (email links point to live `https://thebitcoinbeacon.com/stories/...` URLs; beehiiv write-API is plan-gated on the current plan — issue HTML is drafted locally and pasted, or upgrade the beehiiv plan to unlock `save_post` automation).
