...
The nav topic tabs point to `/topics/<beat>.html` (NOT `/archive.html#beat` anymore). Each topic page shows that beat's **latest 10** stories + an "Archives" link to `/archive/<beat>.html`, the beat's **full** history. `/archive.html` is now a hub of six topic cards. All of these are GENERATED from `catalog.csv` by `_system/build-topic-pages.py` — never hand-edit `public/topics/*`, `public/archive/*`, or `public/archive.html`; edit the catalog (or the script's shell) and re-run. Beats: on-the-ground, money-macro, markets, policy-nation-states, network-mining, opinion.
